...
#define EPS 1e-3           // Tolerance for stability
...
int main(int argc, char *argv[])
{
    ...
    printf("Initial central temperature = %.2f\n", centralTemp);
    printf("--------------------------------------------------------\n\n");
    ...
    while (true) {
        ...
        for (int i = 0;  i < numExternals; i++){
            ...
            int idx = messageFromClient.Index - 1;
            temperature[idx] = messageFromClient.T;
            printf("Temperature of External Process (%d) = %.2f\n", messageFromClient.Index, temperature[idx]);
        ...
        if (stable && !first) {
            // Send done signal with last centralTemp
            struct msg updated_msg = prepare_message(0, centralTemp, true); 
            for (int i = 0;  i < numExternals; i++){
            ...
            printf("System has stabilized at central temperature = %.2f\n", centralTemp);
            break;
        }
        ...
        printf("Updated central temperature = %.2f\n\n", centralTemp);

        // Construct message with updated temperature
        struct msg updated_msg = prepare_message(0, centralTemp, false); 
